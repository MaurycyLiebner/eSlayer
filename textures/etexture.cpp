...
SDL_Texture* generateTextTexture(SDL_Renderer* const r,
    ...
    SDL_Surface* surf;
    if(width) {
        surf = TTF_RenderText_Blended_Wrapped(&font, text.c_str(), text.size(), color, width);
    } else {
        surf = TTF_RenderText_Blended_Wrapped(&font, text.c_str(), text.size(), color, 0);
    }
    if(!surf) {
...
bool eTexture::loadText(SDL_Renderer* const r,
    ...
    }
    {
        ...
        const SDL_FRect dstRect2{0.f, 0.f, float(w), float(h)};
        SDL_RenderTexture(r, tex2, NULL, &dstRect2);
        const SDL_FRect dstRect1{dx, dy, float(w), float(h)};
        SDL_RenderTexture(r, tex1, NULL, &dstRect1);
