...
void eZipLoader::open(const std::string& path) {
    ...
    if(!mArchive) {
        ...
        throw std::runtime_error("Failed to open zip archive " + errStr);
    }
}
...
std::vector<std::byte> eZipLoader::load(const std::string& path) {
    if(!mArchive) {
        throw std::runtime_error("Zip archive not opened");
    }
    ...
    if(zip_stat_index(mArchive, index, 0, &sb) != 0) {
        throw std::runtime_error("Failed to stat file in zip");
    }
